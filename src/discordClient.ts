...
client.on(Events.InteractionCreate, async (interaction: Interaction<CacheType>) => {
    ...
    try {
        if (interaction.isChatInputCommand()) {
            ...
            addLogEntry(user, 'StringSelectMenu', `Received select menu interaction: ${interaction.customId}`);
            
            const [actionPrefix, ...params] = interaction.customId.split(':');
            const originalInteractionId = params[params.length - 1];
            
            addLogEntry(user, 'StringSelectMenu', `Parsed actionPrefix: ${actionPrefix}, originalInteractionId: ${originalInteractionId || 'undefined'}`);

            if (actionPrefix === 'delete-select' && params.length >= 2 && originalInteractionId) {
                const deleteFiles = params[0] === 'true';
                ...
                addLogEntry(user, `/stopspecificseeds (select start)`, `Starting to handle selection for ${originalInteractionId}`);
                try {
                    const selectedHashes = interaction.values;
                    ...
                    addLogEntry(user, `/stopspecificseeds (select error)`, `Error handling torrent selection: ${error instanceof Error ? error.message : String(error)}`);
                    try {
                        await interaction.update({ 
            ...
            addLogEntry(user, 'ButtonInteraction', `Received button interaction: ${interaction.customId}`);
            
            const [actionPrefix, ...params] = interaction.customId.split(':');
            const originalInteractionId = params[params.length -1]; // last param is usually the ID

            addLogEntry(user, 'ButtonInteraction', `Parsed actionPrefix: ${actionPrefix}, originalInteractionId: ${originalInteractionId || 'undefined'}`);
        ...
        addLogEntry('System', 'InteractionError', `Unhandled error for user ${user}: ${error instanceof Error ? error.message : String(error)}`);
